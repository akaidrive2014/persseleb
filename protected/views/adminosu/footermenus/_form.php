...
/* @var $this FootermenusController */
/* @var $model SBFooterMenu */
...
<div class="panel panel-default">
	...
	<div class="panel-body form">
		...
		<?php $form = $this -> beginWidget('CActiveForm', array('id' => $this->modelName.'-form',
 ...
 ?>
		...
		<div class="row">
			...
			<?php echo $form -> dropDownList($model, 'parent_id', CHtml::listData(SBFooterMenu::model() -> findAll(array('order' => 'menu_name ASC')), 'menu_id', 'menu_name'), array('empty' => '--Select--', 'class' => 'form-control')); ?>
...
<script>
	$(function(){
		var action =$("#SBFooterMenu_action.form-control option:selected").val();
		...
		$("#SBFooterMenu_page_action.form-control").change(function(){
		...
		$("#SBFooterMenu_action.form-control").change(function(){
			var action = $(this).val();
			if(action=='link-to-page'){
				$('.page-action').find('select').val(''); 
				$(".page-action,.page").show();
		...
		$("#save").click(function(){
			...
			loading('show');
			var SBFooterMenu_page_content = CKEDITOR.instances['SBFooterMenu_page_content'].getData();
			$.post($('#<?php echo $this->modelName;?>-form').attr('action'),$('#<?php echo $this->modelName;?>-form').serialize()+'&SBFooterMenu%5Bpage_content%5D='+encodeURIComponent(SBFooterMenu_page_content),function(response){
				if(response.success==true){
					...
					if(response.SBFooterMenu_menu_name){ 
						$("#SBFooterMenu_menu_name.form-control").addClass('error'); 
						$('#SBFooterMenu_menu_name_em_').html(response.SBFooterMenu_menu_name).show(); 
					}else{ 
						$("#SBFooterMenu_menu_name").removeClass('error'); 
						$('#SBFooterMenu_menu_name_em_').hide(); 
					}
					if(response.SBFooterMenu_action){ 
						$("#SBFooterMenu_action.form-control").addClass('error'); 
						$('#SBFooterMenu_action_em_').html(response.SBFooterMenu_action).show(); 
					}else{ 
						$("#SBFooterMenu_action").removeClass('error'); 
						$('#SBFooterMenu_action_em_').hide(); 
					} 
					if(response.SBFooterMenu_page_id){ 
						$("#SBFooterMenu_page_id.form-control").addClass('error'); 
						$('#SBFooterMenu_page_id_em_').html(response.SBFooterMenu_page_id).show(); 
					}else{ 
						$("#SBFooterMenu_page_id").removeClass('error'); 
						$('#SBFooterMenu_page_id_em_').hide(); 
					}
					if(response.SBFooterMenu_page_name){ 
						$("#SBFooterMenu_page_name.form-control").addClass('error'); 
						$('#SBFooterMenu_page_name_em_').html(response.SBFooterMenu_page_name).show(); 
					}else{ 
						$("#SBFooterMenu_page_name").removeClass('error'); 
						$('#SBFooterMenu_page_name_em_').hide(); 
					}
					if(response.SBFooterMenu_page_content){ 
						$("#SBFooterMenu_page_content.form-control").addClass('error'); 
						$('#SBFooterMenu_page_content_em_').html(response.SBFooterMenu_page_content).show(); 
					}else{ 
						$("#SBFooterMenu_page_content").removeClass('error'); 
						$('#SBFooterMenu_page_content_em_').hide(); 
					}
					if(response.SBFooterMenu_module){ 
						$("#SBFooterMenu_module.form-control").addClass('error'); 
						$('#SBFooterMenu_module_em_').html(response.SBFooterMenu_module).show(); 
					}else{ 
						$("#SBFooterMenu_module").removeClass('error'); 
						$('#SBFooterMenu_module_em_').hide(); 
					}
					if(response.SBFooterMenu_category_news_id){ 
						$("#SBFooterMenu_category_news_id.form-control").addClass('error'); 
						$('#SBFooterMenu_category_news_id_em_').html(response.SBFooterMenu_category_news_id).show(); 
					}else{ 
						$("#SBFooterMenu_category_news_id").removeClass('error'); 
						$('#SBFooterMenu_category_news_id_em_').hide(); 
					}
...
var editor = CKEDITOR.replace( 'SBFooterMenu_page_content', {
