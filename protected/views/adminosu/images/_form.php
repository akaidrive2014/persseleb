...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<div class="form-group">
		...
		<?php echo $form->textField($model,'image_title',array('size'=>60,'maxlength'=>100,'class'=>'form-control')); ?>
		<?php echo $form->error($model,'image_title'); ?>
	</div>
	<fieldset>
		<legend>Image Digital <span class="required">*</span> (<span style="font-style:italic;color:#8F8F8F;">Recommended Dimensions : 900px x 600px</span>)
		...
		<div class="col-sm-11 col-lg-12">
				<span class="btn btn-success fileinput-button"> <i class="glyphicon glyphicon-plus"></i> <span>Select files...</span> <!-- The file input field used as target for the file upload widget -->
					...
					<?php echo $form->hiddenField($model,'image',array('style'=>'margin-top:-72px !important;','class'=>'form-control')); ?>
				</span>
				<span class="view-logo">
					<?php
					if(!empty($model->image)){
					?>	
					<img width="135" src="<?php echo $this->baseUrl();?>/assets/uploads/images/<?php echo $model->image;?>" style="float:right;">
					<?php }?>
				</span>
...
<script>
	$(function(){
		$("#save").click(function(){
			...
			$.post($('#<?php echo $this->imageModelName;?>-form').attr('action'),$('#<?php echo $this->imageModelName;?>-form').serialize(),function(response){
				if(response.success==true){
					success('show');
					setTimeout(function(){
						success('hide');
					},1000);
					$('#<?php echo $this->imageModelName;?>-grid').yiiGridView('update', { 
						data: $(this).serialize() 
					}); 
					$("#myModal").modal('hide'); 
				}else{
